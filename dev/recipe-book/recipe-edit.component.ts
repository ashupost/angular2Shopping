...
export class RecipeEditComponent implements OnInit, CanDeactivate {
			...
			new ControlGroup(
				{
					...
					amount: new Control(itemAmount,	Validators.compose([Validators.required,hasNumbers]))
				}
			)
		)
	}
	...
	ngOnInit():any {
		...
		let fbRecipeName, fbRecipeImageUrl, fbRecipeContent = '';
		...
		if(this._editMode === 'edit'){
			...
			for (let i = 0; i < this.recipe.ingredients.length; i++){
				fbIngredients.push(new ControlGroup({
							name: new Control(this.recipe.ingredients[i].name, Validators.required),
							amount: new Control(this.recipe.ingredients[i].amount,	Validators.compose([Validators.required, hasNumbers]))
						})
				);
		...
		this.myForm = this._formBuilder.group({
			...
			content: [fbRecipeContent, Validators.required],
